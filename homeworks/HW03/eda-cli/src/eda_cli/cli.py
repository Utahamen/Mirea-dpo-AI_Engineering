...
def head(
    ...
    Аналог команды head в Unix/Linux для CSV-файлов.
    Полезно для быстрого просмотра структуры данных.
    """
    df = _load_csv(Path(path), sep=sep, encoding=encoding)
    
    if n < 0:
        raise typer.BadParameter("Количество строк должно быть неотрицательным числом")
    
    # Выводим первые n строк (или все, если n больше количества строк)
    result_df = df.head(n)
    typer.echo(result_df.to_string(index=False))
    
    # Показываем информацию о том, сколько строк было выведено
    total_rows = len(df)
    shown_rows = len(result_df)
    if shown_rows < total_rows:
        typer.echo(f"\n(Показано {shown_rows} из {total_rows} строк)")
...
def report(
    ...
    min_missing_share: float = typer.Option(0.0, help="Порог доли пропусков, выше которого колонка считается проблемной."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if missing_df.empty:
            ...
            if problematic_columns:
                ...
                for col_name in problematic_columns:
                    missing_share_val = missing_df.loc[col_name, "missing_share"]
                    f.write(f"- `{col_name}`: **{missing_share_val:.1%}** пропусков\n")
                f.write("\n")
        ...
        if not top_cats:
            ...
            f.write(f"Для каждой категориальной колонки выводится top-{top_k_categories} значений.\n\n")
