...
def compute_quality_flags(
    ...
    # Проверка на категориальные колонки с высокой кардинальностью
    # Высокая кардинальность = уникальных значений больше 90% от общего числа строк
    high_cardinality_cols = []
    for col in summary.columns:
        if not col.is_numeric and col.unique > 0:
            # Проверяем, что это категориальная колонка и кардинальность высокая
            unique_ratio = col.unique / summary.n_rows if summary.n_rows > 0 else 0.0
            if unique_ratio > 0.9:
                high_cardinality_cols.append(col.name)
    flags["has_high_cardinality_categoricals"] = len(high_cardinality_cols) > 0
    flags["high_cardinality_categorical_columns"] = high_cardinality_cols

    # Проверка на подозрительные дубликаты в ID-колонках
    # Ищем колонки с именами, содержащими "id" (case-insensitive)
    ...
    if df is not None:
        ...
        for col_name in id_like_cols:
            # Проверяем, есть ли дубликаты в колонке, которая должна быть уникальной
