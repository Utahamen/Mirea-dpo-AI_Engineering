...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool | float | list[str]] = Field(
        ...,
        description="Полный набор флагов качества, включая все эвристики из HW03",
    )
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    включая все эвристики из HW03 (has_constant_columns, has_high_cardinality_categoricals,
    has_suspicious_id_duplicates, has_many_zero_values и т.д.).
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
